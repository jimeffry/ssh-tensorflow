...
def _smooth_l1_loss_base(bbox_pred, bbox_targets, sigma=1.0,name='smooth_l1_loss'):
    '''
...
def smooth_l1_loss_rcnn(bbox_pred, bbox_targets, label, num_classes, sigma=1.0,name='smooth_l1_rcnn_loss'):
    '''
    ...
    '''
    with tf.variable_scope(name):
        outside_mask = tf.stop_gradient(tf.to_float(tf.greater(label, 0),name='out_great_op'),name='out_mask_op')

        bbox_pred = tf.reshape(bbox_pred, [-1, num_classes, 4],name='bbox_pred_reshape')
        bbox_targets = tf.reshape(bbox_targets, [-1, num_classes, 4],name='bbox_target_reshape')

        value = _smooth_l1_loss_base(bbox_pred,
                                    bbox_targets,
                                    sigma=sigma)
        value = tf.reduce_sum(value, 2,name='inside_loss_sum')
        value = tf.reshape(value, [-1, num_classes],name='inside_loss_reshape')

        inside_mask = tf.one_hot(tf.reshape(label, [-1, 1]),
                                depth=num_classes, axis=1,name='inside_label_onehot')

        inside_mask = tf.stop_gradient(
            tf.to_float(tf.reshape(inside_mask, [-1, num_classes])),name='inside_label_mask')

        normalizer = tf.to_float(tf.shape(bbox_pred,name='get_bboxpred_shape')[0],name='get_bachsize')
        bbox_loss = tf.reduce_sum(
            tf.reduce_sum(value * inside_mask, 1,name='select_inside_sum')*outside_mask,name='select_out_sum') / normalizer
